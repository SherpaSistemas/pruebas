...
cursorSQL.execute("TRUNCATE TABLE dbo.inv")#NOMBRE DE LA TABLA EN mssql
cursorSQL.commit()

#07/20/23 0000-00-00
def ordenaFecha(fecha):
    if fecha == '':
        return None
    else:
        fecha = fecha.replace("*", "").split("-")
        return f"{fecha[3]}-{fecha[2]}-{fecha[1]} {fecha[0]}"
